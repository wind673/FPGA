...
u16 ADC_Waveform_Get(u16 *SourceBuff,u16 SourceLenth,double *ResultBuff,u16 ResultLenth)
{
	...
	{
		if(SourceBuff[i] > 4096) return 0; //DMA传输溢出  
	...
	OS_String_Show(0,0,(u8*)"缩放度：",16,1); OS_Decim_Show(0+16*4,0,ZoomX,8,2,16,1); 
	
	OS_String_Show(0,40+16*0,(u8*)"取样点数：",16,1);	OS_Decim_Show(0+16*5,40+16*0,EndX - StartX,8,0,16,1); 
	OS_String_Show(0,40+16*1,(u8*)"起点坐标：",16,1);	OS_Decim_Show(0+16*5,40+16*1,StartX,8,0,16,1);
	OS_String_Show(0,40+16*2,(u8*)"终点坐标：",16,1);	OS_Decim_Show(0+16*5,40+16*2,EndX,8,0,16,1);  
	...
	OS_String_Show(0,100+16*2,(u8*)"Min：",16,1);		OS_Decim_Show(0+16*3,100+16*2,Min*3.3/4096,4,3,16,1);//峰峰值 电压最大值  电压最小值 
	ERROR_HANDLE(16);
	return EndX - StartX;
}
