...
(   
	 ...
	 ,input in_key
	 ,input [31:0]in_test_freq			 //高频数据
	 ,input [31:0]in_test_freq_low   //低频数据
...
begin     
		...
		case(receive_buff[1])
			...
			6:begin    
				send_buff[0] <= 8'hAA; 
				send_buff[1] <= 6; 
				send_buff[2] <= (in_test_freq_low >> 24)&8'hff; 
				send_buff[3] <= (in_test_freq_low >> 16)&8'hff;
				send_buff[4] <= (in_test_freq_low >> 8)&8'hff;
				send_buff[5] <= (in_test_freq_low)&8'hff ; 
				send_buff[6] <= 0;
				send_buff[7] <= 0;
				send_buff[8] <= 0; 
				send_start_flag <= 1; //开始发送
